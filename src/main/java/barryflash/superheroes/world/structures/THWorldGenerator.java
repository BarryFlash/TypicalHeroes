...
public class THWorldGenerator extends WorldGenerator implements IWorldGenerator {
	static Random rand2 = new Random();

	@Override
	public void generate(Random rand, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		int blockX = chunkX * 16 + 1;
		int blockZ = chunkZ * 16 + 1;
		...
		switch (world.provider.getDimension()) {
				...
				break;
			case 0: 
				generateOverworld(world, rand, blockX, blockZ, BiomeSavanna.class);
				generateOre(ModBlocks.TITANIUM_ORE_BLOCK.getDefaultState(), world, rand, chunkX * 16, chunkZ * 16, 16, 44, 4 + rand.nextInt(4), 6);
				generateOre(ModBlocks.URANIUM_ORE_BLOCK.getDefaultState(), world, rand, chunkX * 16, chunkZ * 16, 4, 38, 4 + rand.nextInt(3), 4);
				generateOre(ModBlocks.VIBRANIUM_ORE.getDefaultState(), world, rand, chunkX * 16, chunkZ * 16, 4, 38, 4 + rand.nextInt(4), 8);
				break;
			case 1: 
	...
	private void generateOverworld(World world, Random rand, int blockX, int blockZ, Class<?>... classes) {	
		if ((int) (Math.random() * 1000) == 0) {
			...
			WorldGenerator structure = new KryptoniteShipGen();
		...
		if(world.getWorldType() != WorldType.FLAT) {
			...
			if(classesList.contains(biome)) {
				if ((int) (Math.random() * 800) == 0) {
					WorldGenerator structure = new ThemysciraGen();
					structure.generate(world, rand, pos);
				}
			}
		}
