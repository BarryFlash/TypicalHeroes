...
public class RenderThorIWLayer implements LayerRenderer<AbstractClientPlayer>
{
  
 
    private final RenderLivingBase<?> renderer;
    
    public RenderThorIWLayer(RenderLivingBase<?> rendererIn)
    {
    ...
    {
    	/*scale = 0.0625F;
    	if (ItemThorArmor.isWearingFullSet(entitylivingbaseIn, ModArmor.ThorChest, ModArmor.ThorLeggings, ModArmor.ThorBoots)) {
    		IThor thorcap = entitylivingbaseIn.getCapability(ThorProvider.THOR_CAP, null);
    		if (thorcap.isLightningForm()) {
    				ModelThorLightning model = new ModelThorLightning(scale);
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(Reference.MODID + ":textures/models/armor/ThorLightning.png"));
