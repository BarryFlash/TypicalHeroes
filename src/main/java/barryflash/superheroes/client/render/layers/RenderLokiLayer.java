...
import barryflash.superheroes.models.ModelLokiCape;
...
public class RenderLokiLayer implements LayerRenderer<AbstractClientPlayer> {

	
	@Override
	public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount,
			float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		if (ItemLokiArmor.isWearingFullSet(entitylivingbaseIn)) {
			ILoki lokicap = entitylivingbaseIn.getCapability(LokiProvider.LOKI_CAP, null);
			if (!lokicap.isInvisible()) {
			ModelBiped model = new ModelLokiCape();
		...
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		 ...
		 GlStateManager.translate(0.0F, -0.05F, 0.3F);
          ...
          GlStateManager.rotate(12.0F + f2 / 3.5F + f1, 8.0F, 0.0F, 0.0F);
         GlStateManager.rotate(180.0F, 0.0F, 1.0F, 0.0F);
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(Reference.MODID + ":textures/models/armor/loki.png"));
