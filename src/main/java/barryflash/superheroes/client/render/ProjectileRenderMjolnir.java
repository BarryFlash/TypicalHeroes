...
public class ProjectileRenderMjolnir extends Render<EntityMjolnir> {

	private ModelBase model = new ModelMjolnir();

	public ProjectileRenderMjolnir(RenderManager renderManager) {
		super(renderManager);
	}

	public ProjectileRenderMjolnir(RenderManager renderManager, ModelBase model) {
		this(renderManager);
		this.model = model;
	}

	@Override
	public void doRender(EntityMjolnir entity, double x, double y, double z, float entityYaw, float partialTicks) {
	...
	@Override
	protected ResourceLocation getEntityTexture(EntityMjolnir entity) {
		return new ResourceLocation(Reference.MODID, "textures/entity/thor_mjolnir.png");
	}
}	