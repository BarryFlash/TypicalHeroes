...
public class ContainerDnaExtractor extends Container {
	TileEntityDnaExtractor te;
	private int energy, cookTime;
	public ContainerDnaExtractor(InventoryPlayer inventoryPlayer, TileEntityDnaExtractor te) {
		this.te = te;
		if (te.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null)) {
			IItemHandler inventory = te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
			
			addSlotToContainer(new SlotNanoCompacterInput(inventory, 0, 47, 34));
			addSlotToContainer(new SlotNanoCompacterOutput(inventory, 1, 105, 35));
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 9; x++) {
