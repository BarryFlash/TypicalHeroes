...
public class ContainerNanoCompacter extends Container {
	TileEntityNanoCompacter te;
	private int energy, cookTime;
	public ContainerNanoCompacter(InventoryPlayer inventoryPlayer, TileEntityNanoCompacter te) {
		this.te = te;
		if (te.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null)) {
			IItemHandler inventory = te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
			
			addSlotToContainer(new SlotNanoCompacterInput(inventory, 0, 47, 34));
			addSlotToContainer(new SlotNanoCompacterBottle(inventory, 1, 75, 54));
			addSlotToContainer(new SlotNanoCompacterOutput(inventory, 2, 105, 35));
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 9; x++) {
