...
public class EntityCaptainAmericaShield extends EntityThrowable {
	...
	public EntityCaptainAmericaShield(World WorldIn, EntityLivingBase throwerIn) {
		super(WorldIn, throwerIn);
	}

	public EntityCaptainAmericaShield(World WorldIn, double x, double y, double z) {
		super(WorldIn, x, y, z);
	}
    ...
    public void onUpdate() {
    	...
    	if (this.ticksExisted > 120) {
    		EntityItem capItem = new EntityItem(world, this.getThrower().posX, this.getThrower().posY + 0.1, this.getThrower().posZ, new ItemStack(ModItems.cap_shield, 1));
			
			world.spawnEntity(capItem);
			this.setDead();
    	}
    	}
    }
   ...
   public void setHeadingFromThrower(Entity entityThrower, float rotationPitchIn, float rotationYawIn, float pitchOffset, float inaccuracy)
   {
	...
	protected void onImpact(RayTraceResult result) {
			...
			if (result.entityHit != null) {
				if (result.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, this.getThrower()), 15)) {
					/*if (contacted == false) {
						contacted = true;
					}*/
